...
// this is what is working 
    {
        ...
        // https://developer.amazon.com/public/solutions/alexa/alexa-skills-kit/docs/smart-home-skill-api-reference#discoverappliancesresponse
        actions: ['turnOn', 'turnOff','channelController'],
...
function stepSpeaker(applianceId, delta) {
    log('DEBUG', `stepSpeaker (applianceId: ${applianceId}), delta: ${delta}`);

    // Call device cloud's API to set percentage delta

    return generateResponse('stepSpeakerConfirmation', {});
}
function channelController(applianceId, delta) {
    log('DEBUG', `channelController (applianceId: ${applianceId}), delta: ${delta}`);

    // Call device cloud's API to set percentage delta

    return generateResponse('channelControllerConfirmation', {});
}
function thermostatController(applianceId, delta) {
    log('DEBUG', `thermostatController (applianceId: ${applianceId}), delta: ${delta}`);

    // Call device cloud's API to set percentage delta

    return generateResponse('thermostatControllerConfirmation', {});
}
...
function handleControl(request, callback) {
    ...
    switch (request.header.name) {
        ...
        case 'stepSpeakerRequest': {
            ...
            response = stepSpeaker(applianceId, userAccessToken, delta);
            break;
        }
        case 'channelControllerRequest': {
            const delta = request.payload.deltaPercentage.value;
            if (!delta) {
                const payload = { faultingParameter: `deltaPercentage: ${delta}` };
                callback(null, generateResponse('UnexpectedInformationReceivedError', payload));
            ...
            response = channelController(applianceId, userAccessToken, delta);
            break;
        }
         case 'thermostatControllerRequest': {
            const delta = request.payload.deltaPercentage.value;
            if (!delta) {
                const payload = { faultingParameter: `deltaPercentage: ${delta}` };
                callback(null, generateResponse('UnexpectedInformationReceivedError', payload));
            ...
            response = thermostatController(applianceId, userAccessToken, delta);
            break;
        }
